...
const BSEquityTable = ({ equity, date }) => {
  // Create an array of objects with accountId, accountName, and totalAmount
  const accountAmountsArray = equity.reduce((acc, eq) => {
    const accountId = eq.accountId;
    const accountName = eq.accountName;
    const amount = parseFloat(eq.amount) * (eq.dc === "credit" ? 1 : -1);
      ...
      <div className="overflow-x-auto">
        <Table hoverable>
          <Table.Head className="bg-gray-800">
            <Table.HeadCell className="text-center">Equity</Table.HeadCell>
          ...
          <Table.Body>
            <Table.Row className="bg-gray">
              <Table.Cell className="font-bold ">Total Equity: </Table.Cell>
